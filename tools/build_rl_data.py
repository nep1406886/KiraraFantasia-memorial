...
# reconfigure, not rewrap: a replacement TextIOWrapper lets the original be
# garbage-collected, which closes the underlying buffer and kills every later
# print (imported build_skill_playback died on exactly that).
sys.stdout.reconfigure(encoding="utf-8", errors="replace")

ROOT = Path(__file__).resolve().parent.parent
RAW = ROOT / "site" / "asset" / "rl" / "_raw"
OUT = ROOT / "site" / "asset" / "rl"
...
def load_gacha_data() -> dict:
    text = (ROOT / "site" / "asset" / "gacha" / "cards.js").read_text(encoding="utf-8")
...
def build_enemies() -> None:
    ...
    manifest = json.loads((ROOT / "site" / "asset" / "models" / "manifest.json")
...
def build_playable_roster() -> None:
    ...
    path = ROOT / "site" / "game" / "rl" / "rosterids.js"
...
ENCOUNTER_TABLE = [
    ...
    },
    {
        ...
        # level 3 -> 2 (2026-09-18 T31 校准): elv3 的杂兵把 §9 胜率压到
        # 41.7%（带 44-64）；elv2 读 61.6% 入带。bossHp 目标不变，重新生成时
        # hpScale 按新层号重算，斩杀线保持 92s 左右（§6 带 90-150）。
        "playerLevel": 57, "level": 2, "tier": 0.75,
